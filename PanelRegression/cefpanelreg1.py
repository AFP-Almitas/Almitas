...
class CEFpanelreg:
    ...
    def result(self,
        ...
        self.data['terminationdate'] = pd.to_datetime(
            self.data['terminationdate'])
        ...
        # compute return
        self.data['lpriceclose'] = self.data[[
            'ticker', 'priceclose']].groupby('ticker').shift(1)
        self.data['ret'] = self.data['priceclose']/self.data['lpriceclose'] - 1

        # change in discount
        # self.data['lpd'] = self.data[['ticker','pd']].groupby('ticker').shift(5)  #1
        self.data['lpd'] = self.data[['ticker', 'pd']
                                     ].groupby('ticker').shift(1)  # 2
        self.data['cd'] = self.data['pd'] - self.data['lpd']

        # weekly change in discount
        self.data['lpd5'] = self.data[['ticker', 'pd']
                                      ].groupby('ticker').shift(5)  # 2
        self.data['cd5'] = self.data['pd'] - self.data['lpd5']

        # age
        self.data['age'] = (self.data['date'] -
                            self.data['inceptiondate']).dt.days
        self.data['age'] = np.log(self.data['age'])

        # time to maturity
        #self.data.loc[self.data['terminationdate'].isna() & (self.data['marketstatus']=='ACTV')]['terminationdate'] = '2019-12-31'
        self.data['tomaturity'] = (
            self.data['terminationdate']-self.data['date']).dt.days
        self.data['tomaturity'] = np.log(self.data['tomaturity'])

        # nav change
        self.data['lnav'] = self.data[['ticker', 'nav']
                                      ].groupby('ticker').shift(1)  # 1a/2
        # self.data['lnav'] = self.data[['ticker','nav']].groupby('ticker').shift(5)      #1b
        self.data['navchg'] = self.data['nav'] - self.data['lnav']

        # select only rows that has valid weekly data
        # self.data = self.data.loc[self.data.validweekly==True]     #2
        ...
        for var in var_pit:
            ...
            self.data[variable+'_' +
                      str(lag)] = self.data.groupby(['ticker'])[variable].shift(lag)

        # Normalized variables
        def decor(func):
            def wrapper(x, y, z, var):
                print("Regressing on "+str(var)+" using "+str(z) +
                      "-day "+func.__name__+" from lag "+str(y)+"...")
                return func(x, y, z, var)
            return wrapper

        @decor
        def mean(df, lag, length, var):
            df[variable+'_'+str(lag)+'_'+f+str(length)] = df.groupby(
                'ticker')[variable].shift(lag).rolling(length).mean()

        @decor
        def std(df, lag, length, var):
            df[variable+'_'+str(lag)+'_'+f+str(length)] = df.groupby(
                'ticker')[variable].shift(lag).rolling(length).std()

        @decor
        def sum(df, lag, length, var):
            df[variable+'_'+str(lag)+'_'+f+str(length)] = df.groupby(
                'ticker')[variable].shift(lag).rolling(length).sum()
        ...
        self.result = self.__fitreg(
            self.data, start_datetime, end_datetime, y, var_pit, var_norm, fix, cluster, self.c)
    ...
    def summary(self):
        ...
        print(pd.concat([self.sumstat['Coefficient'],
                         self.sumstat['t-stat']], axis=1))
    ...
    def __CheckValidLag(self, lim=8):
        ...
        self.data['dif'] = self.data['dif'].fillna(
            pd.Timedelta(seconds=0)).dt.days.astype(int)
    ...
    def __fitreg(self,
        ...
        # filter columns
        dt = dt[y + ['year', 'ticker'] +
                [col for col in dt.columns[self.c:]] + fix]
        ...
        if len(fix) == 0 and len(cluster) == 0:
            ...
            fit1 = mod.fit(cov_type='heteroskedastic',
                           cluster_time=False, cluster_entity=False)
            print(fit1)
            return fit1

        if len(fix) == 1:
            mod = PanelOLS.from_formula(
                y[0] + '~1+' + x + '+' + fix[0], data=dt)
            if len(cluster) == 0:
                ...
                fit1 = mod.fit(cov_type='clustered',
                               cluster_time=True, cluster_entity=False)
                print(fit1)
                return fit1
            elif cluster == ['ticker']:
                fit1 = mod.fit(cov_type='clustered',
                               cluster_time=False, cluster_entity=True)
                print(fit1)
                return fit1
            elif cluster == ['year', 'ticker'] or cluster == ['ticker', 'year']:
                fit1 = mod.fit(cov_type='clustered',
                               cluster_time=True, cluster_entity=True)
                print(fit1)
                return fit1
            else:
